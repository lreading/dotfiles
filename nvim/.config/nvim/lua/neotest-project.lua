...
local function is_test_file(path)
  return path:match("__tests__")
    or path:match("%.spec%.[cm]?[jt]sx?$")
    or path:match("%.test%.[cm]?[jt]sx?$")
end
...
function M.vitest_command(path)
  local settings = merged_settings(path)
  return settings.vitest.command and resolve_command(path, settings.vitest.command)
    or local_bin(path, "vitest")
end

function M.has_vitest_project(path)
  local settings = merged_settings(path)
  return settings.vitest.enabled ~= false
    and has_dependency(path, { "vitest", "@vitest/ui", "@vitest/coverage-v8" })
end
...
function M.jest_command(path)
  local settings = merged_settings(path)
  return settings.jest.command and resolve_command(path, settings.jest.command)
    or local_bin(path, "jest")
end
...
function M.mocha_command(path)
  local settings = merged_settings(path)
  return settings.mocha.command and resolve_command(path, settings.mocha.command)
    or local_bin(path, "mocha")
end
